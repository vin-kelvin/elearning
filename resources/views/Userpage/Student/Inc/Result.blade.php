            ...
            <main class="col-md-9">
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        @foreach ($assignedSubjects as $result)
                            @php
                                $question = QuestionBank::find($result->question_id);
                                $subjectName = $subjectNames[$result->question_id];
                            @endphp
                            @if ($question && $subjectName)
                                @if ($loop->first || $subjectName !== $subjectNames[$assignedSubjects[$loop->index - 1]->question_id])
                                <ul class="list-group ">
                                    <li class="list-group-item bg-warning ">
                                    <a href="#" class="list-group-item list-group-item-action mt-2 shadow" data-bs-toggle="modal" data-bs-target="#subjectModal{{ $question->id }}">
                                        {{ ucwords($subjectName) }} 
                                    ...
                                    <!-- Subject Modal -->
                                    <div class="modal fade" id="subjectModal{{ $question->id }}" tabindex="-1" aria-labelledby="subjectModal{{ $question->id }}Label" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title " id="subjectModal{{ $question->id }}Label">Available Results For {{ strtoupper($subjectName) }}</h5>
                                                                ...
                                                                <tbody>
                                                                    @foreach ($assignedSubjects as $result)
                                                                        @php
                                                                            $question = QuestionBank::find($result->question_id);
                                                                        @endphp
                                                                        @if ($question && $subjectName === $subjectNames[$result->question_id])
                                                                            <tr>
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <a href="{{ url('testresult/pdf') }}" class="btn btn-outline-dark">
                                                        Download PDF
                                                    </a>
                ...
                <nav class="d-md-block bg-light sidebar collapse">
                    <div class="position-sticky pt-3">
                        <ul class="nav flex-column">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" href="/Inc/Result">
